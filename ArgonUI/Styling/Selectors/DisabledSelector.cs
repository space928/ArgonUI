...
/// <summary>
/// A style selector which selects elements which are not enabled (elements where <see cref="UIElement.Enabled"/> is <see langword="false"/>).
/// </summary>
/// <param name="invert">Whether the behaviour of this selector should be inverted.</param>
public class DisabledSelector(bool invert = false) : IStyleSelector, IFlattenedStyleSelector
{
    private readonly bool invert = invert;

    /// <summary>
    /// When inverted, this selector selects all elements which are enabled.
    /// </summary>
    ...
    private bool SelectElement(UIElement elementTree) => !elementTree.Enabled ^ invert;
    ...
    {
        if (propertyName == nameof(UIElement.Enabled))
